...
- **File tree, tabs, fuzzy file finder** (`Ctrl+O`): the chrome you expect from a GUI editor. Two open files with the same name show their parent folder in the tab. The tree shows git status at a glance — new, modified, and conflicted files are tinted, folders containing changes get a dot — and can create, rename, and delete files in place.
