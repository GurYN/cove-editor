...
type State struct {
	...
	history       []line // Cove patch: lines scrolled off the primary screen
}

// Cove patch: scrollback cap.
...
func (t *State) HistoryCell(i, x int) Glyph {
	if i >= 0 && i < len(t.history) && x >= 0 && x < len(t.history[i]) {
		return t.history[i][x]
	}
...
func (t *State) scrollUp(orig, n int) {
	n = clamp(n, 0, t.bottom-orig+1)
	// Cove patch: lines leaving the top of the primary screen go to
	// scrollback (alt screen and inner scroll regions don't).
	if orig == 0 && t.mode&ModeAltScreen == 0 {
		for i := 0; i < n && i < len(t.lines); i++ {
			ln := make(line, len(t.lines[i]))
			copy(ln, t.lines[i])
			t.history = append(t.history, ln)
		}
		if len(t.history) > historyMax {
			t.history = t.history[len(t.history)-historyMax:]
		}
	}
